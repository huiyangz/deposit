...
public interface IFmDateBcc {
    /** count数据库表记录根据入参com.dcits.deposit.facade.eo.FmDateEO中的属性字段组合 **/
    long countByEo(FmDateEO eo);

    /** remove数据库表记录根据入参com.dcits.deposit.facade.eo.FmDateEO中的属性字段组合 **/
    ...
    /** create数据库表记录，主键和EO对象中不允许为空的字段必填，其它可为空字段可选填，执行时根据com.dcits.deposit.facade.eo.FmDateEO中不为空的属性写入数据库**/
    int createSelective(FmDateEO eo);

    /** find数据库表记录根据入参com.dcits.deposit.facade.eo.FmDateEO中的属性字段组合 **/
    List<FmDateEO> findByEo(FmDateEO eo);

    /** find 根据主键: 法人 **/
    FmDateEO findByPrimaryKey(String company);

    /**  根据主键: 法人执行更新记录操作，仅更新入参com.dcits.deposit.facade.eo.FmDateEO中不为空的属性字段 **/
    ...
    FmDateEO findByCompany(Company company);
}